...
# Source files hash:
SOURCE_DIGEST=$(find $DIR/src $DIR/contrib/**-cmake -type f  | grep -vE '*.md$' | xargs md5sum | awk '{ print $1 }' | sort | md5sum | awk '{ print $1 }')
...
BUILD_DIGEST=$(echo $SOURCE_DIGEST-$MODULES_DIGEST-$DOCKER_DIGEST | md5sum | awk '{ print $1 }')
